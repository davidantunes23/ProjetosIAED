...
int inputp(char *input, Stop *stop) {
    ...
    if (arg == 1 || arg == 3)
    {
        stop->name = malloc((strlen(stopname)+1) * sizeof(char));
...
int inputc(char *input, Line *line, char *inv) {
    ...
    if (arg == 1 || arg == 2)
    {
        line->name = malloc((strlen(linename) + 1) * sizeof(char));
...
void createLineArr(Stop *stop, Line **headLine, char **arr) {
    ...
    while (templine != NULL) {
        ...
        while (templink != NULL) {
            if (templink->origin == stop || templink->destination == stop) {
                arr[i] = malloc(BUFSIZ*sizeof(char));
...
void commandi(Stop **headStop, Line **headLine) {
    ...
    while (tempstop != NULL) {
        ...
        if (linesNum > 1) {
            arr = malloc(linesNum * sizeof(char*));
...
void commandr(char *input, Line **headLine, Line **tailLine) {
    Line *auxline = malloc(sizeof(Line)), *line;
...
int main() {
    ...
    while (input[0] != 'q')
    {
        ...
        else if (input[0] == 'e') commande(input, &headStop, &tailStop, &headLine);
    }
